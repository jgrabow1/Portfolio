...
void rm(Node **root, Node **rootC, char *line) //Removes a file
{
	Node *rTemp = NULL;
	Node *rTempC = NULL;
	...
	if (depth == 0)
	{
		...
		else if (strcmp(rTempC->childPtr->name, dirName) != 0)
		{
			...
			{
				if (strcmp(rTempC->siblingPtr, dirName) == 0 && rTempC->fType == 'F')
				{
					break;
				}
		...
		if (depth == 1)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						if (strcmp(rTemp->siblingPtr, dirName) == 0 && rTemp->fType == 'F')
						{
							break;
						}
